...
def render_new_session(profile: Dict[str, Any]) -> None:
    ...
    if mode == "manual":
        ...
        if (
            ...
            or analyze_frame is None
            or av is None
            or cv2 is None
            or WebRtcMode is None
            or RTCConfiguration is None
        ):
            st.error("Webcam stack unavailable. Install streamlit-webrtc, av, opencv, and deepface.")
            ...
            try:
                ...
                st.error(
                    "Unable to initialize the webcam component in this environment. "
                    "Switch to manual mode or verify your streamlit-webrtc installation."
                )
            ...
            if webrtc_ctx and webrtc_ctx.video_processor:
                latest_emotion = getattr(webrtc_ctx.video_processor, "last_emotion", None)

            if (
                webrtc_ctx
                and WebRtcConnectionState is not None
                and webrtc_ctx.state.connection_state == WebRtcConnectionState.FAILED
            ):
                st.error(
                    "Live video connection failed. This can happen if the network blocks WebRTC or the camera is busy."
                )
                st.caption("Try reloading, switching browsers, or closing other apps using the webcam.")
                st.session_state["mode"] = None
                webrtc_ctx = None
                latest_emotion = None
            elif webrtc_ctx and not webrtc_ctx.state.playing:
                st.info(
                    "Webcam stream initializing… If the preview stays dark, confirm camera access is granted."
                )

            status_placeholder.write(f"Detected: {latest_emotion or 'None'}")

            if st.button("Lock in Mood and Get Recommendation"):
                st.session_state["detected_mood"] = latest_emotion
