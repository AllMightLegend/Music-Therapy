    ...
    if mode == "manual":
        ...
        if webrtc_streamer is None or analyze_frame is None or av is None or cv2 is None:
            ...
            if webrtc_ctx:
                ...
                elif webrtc_ctx.video_receiver and webrtc_ctx.video_receiver.stats:
                    stats = webrtc_ctx.video_receiver.stats
                    received = stats.get("framesReceived") or stats.get("framesDecoded")
                    if not received:
                        st.warning(
                            "No frames are being received yet. If it stays blank, reload the page or toggle permissions."
                        )
