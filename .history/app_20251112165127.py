...
def render_progress_dashboard(profile: Dict[str, Any]) -> None:
    ...
    sessions_this_month = history_df[history_df["timestamp"].dt.to_period("M") == pd.Timestamp.today().to_period("M")]
    ...
    with dist_col:
        ...
        if feedback_counts.empty:
            ...
            centre_circle = plt.Circle((0, 0), 0.55, fc="white" if st.session_state.get("theme", "light") == "light" else "black", alpha=0.35)
    ...
    with streak_col:
        ...
        consecutive_positive = (
            history_df.sort_values("timestamp")["feedback_emoji"].str.lower().eq("happy")[::-1].cumprod().sum()
        )
        st.markdown(
            f"""
            <div class="insight-card">
                ...
                <p><strong>Top Target Mood:</strong><br/>{history_df['target_mood'].mode().iat[0].title()}</p>
            </div>
            """,
            unsafe_allow_html=True,
        )
