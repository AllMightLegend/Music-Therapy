...
def render_new_session(profile: Dict[str, Any]) -> None:
    ...
    if mode == "manual":
        ...
        realtime_available = all(dep is not None for dep in (webrtc_streamer, av))
        ...
        if realtime_available and detector_available:
            ...
            if cv2 is None:
                st.info("OpenCV overlay support is unavailable; detections won't render on the video feed.")
        else:
