...
except Exception as exc:  # allow app to run without webcam stack and surface error
    webrtc_import_error = repr(exc)
...
def render_new_session(profile: Dict[str, Any]) -> None:
    ...
    if start_col.button("Start with Webcam 📹", key="session_start_webcam"):
        st.session_state["mode"] = "webcam"
    if manual_col.button("Start with Manual Input 👆", key="session_start_manual"):
    ...
    if mode == "manual":
        ...
        if st.button("Get Recommendation", key="manual_get_recommendation"):
            st.session_state["detected_mood"] = manual_choice
            detected = manual_choice
        ...
        if webrtc_streamer is None or analyze_frame is None or av is None or cv2 is None:
            ...
            if webrtc_import_error:
                st.caption(f"Import error: {webrtc_import_error}")
        else:
            ...
            try:
                ...
                st.error(
                    "The webcam component could not be initialized in this environment. "
                    "Please switch to manual mode or check your 'streamlit-webrtc' installation."
                )
            ...
            if webrtc_ctx:
                ...
                if webrtc_ctx.state.connection_state == WebRtcConnectionState.FAILED:
                    st.error(
                        "Live video connection failed to establish. This can happen if the network blocks WebRTC or the camera is already in use."
                    )
                    st.caption(
                        "Try reloading the page, switching browsers, or closing other applications using the webcam. "
                    ...
                    st.info(
                        "Webcam stream initializing... if the preview stays dark, confirm camera access is allowed and no other application is using it."
                    )
            st.write(f"Detected: {last or 'None'}")

            if st.button("Lock in Mood and Get Recommendation", key="webcam_get_recommendation"):
                st.session_state["detected_mood"] = last
                detected = last

    detected = detected or st.session_state.get("detected_mood")
    if detected:
        st.markdown(
            f"""
        ...
        if playlist_df.empty:
            ...
            feedback = None
            if c1.button("😞 Not Effective", key="session_feedback_sad"):
                feedback = "sad"
            if c2.button("😐 Neutral", key="session_feedback_neutral"):
                feedback = "neutral"
            if c3.button("😊 Great", key="session_feedback_happy"):
                feedback = "happy"

            if feedback is not None and "current_playlist" in st.session_state:
                playlist_df = st.session_state["current_playlist"]
                playlist_lines = []
                for _, track_row in playlist_df.iterrows():
                    track = str(track_row.get("track") or "Unknown Track")
                    artist = str(track_row.get("artist") or "Unknown Artist")
                    playlist_lines.append(f"{track} — {artist}")
                playlist_text = "\n".join(playlist_lines)
                ...
                st.session_state["detected_mood"] = None
                st.session_state["mode"] = None
                st.session_state.pop("manual_mood_select", None)
                st.session_state.pop("current_playlist", None)
                trigger_rerun()
...
def render_progress_dashboard(profile: Dict[str, Any]) -> None:
    ...
    def _playlist_items(raw: Optional[str]) -> List[str]:
        if raw is None:
            return []
        text = str(raw).strip()
        if not text:
        ...
        if isinstance(parsed, dict):
            ...
            if parsed_df is not None and not parsed_df.empty:
                for _, track_row in parsed_df.iterrows():
                    track = str(track_row.get("track") or "Unknown Track")
                    artist = str(track_row.get("artist") or "Unknown Artist")
                    items.append(f"{track} — {artist}")
        elif isinstance(parsed, list):
            for entry in parsed:
                if isinstance(entry, dict):
                    track = str(entry.get("track") or "Unknown Track")
    ...
    ordered_history = history_df.sort_values("timestamp", ascending=False)
    for _, row in ordered_history.iterrows():
        playlist_items = _playlist_items(row.get("playlist"))
        if playlist_items:
            playlist_html = "".join(f"<li>{item}</li>" for item in playlist_items)
        else:
            playlist_html = "<li class='session-log-empty'>No playlist recorded.</li>"

        feedback_key = str(row.get("feedback_emoji") or "").lower()
        feedback_display = feedback_labels.get(feedback_key, None)
        if feedback_display is None:
            feedback_display = "Feedback pending"

        timestamp_value = row["timestamp"]
        if isinstance(timestamp_value, pd.Timestamp):
            timestamp_str = timestamp_value.strftime("%b %d, %Y · %I:%M %p")
        else:
            timestamp_str = str(timestamp_value)

        start_label = str(row.get("start_mood") or "—").title()
        target_label = str(row.get("target_mood") or "—").title()
        ...
        st.markdown(
            ...
            <div class="session-log-card">
                <div class="session-log-header">
                    <div>
                        <div class="session-log-date">{timestamp_str}</div>
                        <div class="session-log-moods">Start: {start_label} → Target: {target_label}</div>
                    </div>
                    <div class="session-log-feedback">{feedback_display}</div>
                </div>
                <ul class="session-log-playlist">
                    {playlist_html}
                </ul>
            </div>
            """,
