...
try:
    from streamlit_webrtc import (
        VideoTransformerBase as _VideoTransformerBase,
...
def render_new_session(profile: Dict[str, Any]) -> None:
    ...
    if mode == "manual":
        ...
        realtime_available = webrtc_streamer is not None
        detector_available = analyze_frame is not None

        if realtime_available and detector_available:
            if VideoTransformerBase is not None:

                class EmotionTransformer(VideoTransformerBase):  # type: ignore[misc,valid-type]
                    def __init__(self) -> None:
                        self.last_emotion: Optional[str] = None

                    def transform(self, frame: "av.VideoFrame") -> "np.ndarray":  # type: ignore[name-defined]
                        image = frame.to_ndarray(format="bgr24")
                        emotion = analyze_frame(image)
                        if emotion:
                            normalized = str(emotion).strip().lower()
                            self.last_emotion = normalized
                            if cv2 is not None:
                                cv2.putText(
                                    image,
                                    normalized.title(),
                ...
                if (
                    ctx
                    and ctx.video_transformer
                    and getattr(ctx.video_transformer, "last_emotion", None)
                ):
                    latest = ctx.video_transformer.last_emotion  # type: ignore[attr-defined]
                    if latest:
                        normalized = str(latest).strip().lower()
                        if normalized not in MOOD_OPTIONS:
                            normalized = "neutral"
                        if st.session_state.get("last_detected_emotion") != normalized:
                            st.session_state["last_detected_emotion"] = normalized
                ...
                st.warning(
                    "Upgrade `streamlit-webrtc` to a version that exposes `VideoTransformerBase` "
                    "or install `av` to enable realtime mood detection overlays."
                )
        else:
            ...
            if snapshot is not None and detector_available:
                try:
                    ...
                    emotion = analyze_frame(frame_bgr)
                    if emotion:
        ...
        normalized_last = (str(last).strip().lower() if last else None) or None
        if normalized_last and normalized_last not in MOOD_OPTIONS:
            st.warning(
                f"Detected mood '{normalized_last}' is not part of the supported set. "
                "Defaulting to neutral."
            )
            normalized_last = "neutral"
